...
这部分内容主要介绍公共交换电话网络（PSTN），涵盖网络组成、存在问题、调制解调器及数字用户线路（DSL）相关内容。

1. **网络组成**：PSTN由本地回路、干线和交换局构成。本地回路连接用户终端和电话局，通常使用模拟双绞线；干线用于连接不同交换局，多采用数字光纤，因其能高速、长距离传输信号。交换局负责连接不同线路，实现电话呼叫的路由选择。
2. **存在问题**：PSTN存在衰减、延迟畸变和噪声等问题。衰减使信号强度随传输距离增加而减弱；延迟畸变因信号不同频率成分传播速度有差异，导致接收信号失真；噪声会干扰信号传输，降低通信质量。
3. **调制解调器**：在PSTN中，由于本地回路是模拟线路，而计算机等设备传输的是数字信号，所以需要调制解调器进行数模转换。调制解调器依据不同的V系列协议工作，这些协议规定了调制方式和速率等参数。例如V.32协议采用QAM调制技术，速率可达9600bps；V.34协议进一步提升调制技术，速率最高可达28.8kbps。
4. **数字用户线路（DSL）**：DSL利用数字信号传输技术，在不改变本地回路物理线路的前提下提高传输速率。其中ADSL（非对称数字用户线路）应用广泛，它在一对双绞线上实现不对称传输，下行速率较高，适合用户从网络下载数据；上行速率相对较低，用于用户上传数据，满足多数互联网应用场景需求。 
5. **ADSL调制方案（DMT）**：离散多音频调制（DMT）将1.1MHz的带宽划分为256个4312.5Hz的信道。其中，信道0用于传统电话业务（POTS）；1 - 5号信道不使用，以避免语音信号和数据信号相互干扰；剩余250个信道中，1个用于上行控制，1个用于下行控制，248个用于用户数据传输。每个信道采用类似V.34的调制方案，采样率为4000波特，持续监测信道质量并调整数据速率，实际数据通过QAM调制传输，最高可达15比特/波特。相关标准有ANSI T1.413和ITU G.992.1，不同服务等级的速率有所不同，如标准服务为下行512kbps、上行64kbps；高级服务为下行1Mbps、上行256kbps 。
6. **波分复用（WDM和DWDM）**：在光纤通信中，由于目前无法实现更快的电 - 光转换，单根光纤带宽虽约25000GHz，但利用率受限。波分复用（WDM）和密集波分复用（DWDM）技术通过在一根光纤上同时传输多个不同波长的光信号，提高光纤的传输容量。
7. **时分复用**
    - **T1载波**：T1载波的数据传输速率为1.544Mbps，可承载24个语音信道。每个信道包含1位用于帧同步（固定模式01010...）、7位语音采样（速率为56kbps），此外还有用于信令的24位（24×8000帧/秒 = 192kbps） 。当T1系统完全用于数据传输时，仅23个信道用于数据，第24个信道用于同步。
