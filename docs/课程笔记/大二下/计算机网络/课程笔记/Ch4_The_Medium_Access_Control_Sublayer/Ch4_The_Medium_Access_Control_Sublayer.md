...
#### 相关机制
 - **BPDU（网桥协议数据单元，38字节）**：网桥每隔2秒从所有端口组播BPDU消息（目的MAC地址为01:80:c2:00:00:00 ，该消息不转发），内容包含自身ID（基于MAC地址）、认为的根网桥ID、到根网桥的距离 。
 - **选举根网桥**：全网选择ID最低的网桥作为根网桥。通过交换足够多BPDU消息后，所有网桥会就根网桥达成一致。
 - **确定根端口**：每个网桥记住到根网桥的最短路径，若有多条等价路径，选择经过ID最低网桥的路径，该路径对应的端口为根端口。
 - **确定指定端口**：网桥每个端口对应一个局域网，离根网桥最近的端口（等价时选ID最小的）为该局域网的指定端口。既非根端口也非指定端口的会被阻塞，不收发数据。
 - **动态适应**：算法在网络正常运行时持续运行，自动检测拓扑结构变化并更新生成树。相关标准IEEE 802.1D对根端口、指定端口、阻塞端口进行了定义规范。 